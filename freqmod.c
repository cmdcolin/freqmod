...
int main() {
    ...
    while((ch = getch())) {
        ...
        if(third_mod==0) {
            float c1=cos(p2+osc2_t);
            if(osc2_type==1) c1 = c1<=0?-1:1;
            if(osc2_type==2) c1 = osc2 * (1/osc2 - abs(index % (int) (osc2/2) - 1/osc2) );
            float c2=cos(p1+I_t*c1+osc2_t);
            if(osc1_type==1) c2 = c2<=0?-1:1;
            if(osc1_type==2) c2 = osc1 * (1/osc1 - abs(index % (int) (osc1/2) - 1/osc1) );
            buffer[index]=A*c2;
        }
            ...
            if(osc3_type==2) c3 = osc3 * (1/osc3 - abs(index % (int) (osc3/2) - 1/osc3) );
            buffer[index]=A*c3;
        }
        else if(third_mod==2) {
            float c1=cos(p2);
