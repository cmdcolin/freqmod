...
int main() {
    ...
    while((ch = getch())) {
        ...
        else if(ch=='c') { osc3_t+=.01; }
        else if(ch=='v') { osc3_t-=.01; }
        else if(ch=='e') { osc1_t+=.01; }
        else if(ch=='r') { osc1_t-=.01; }
        else if(ch=='t') { osc1_type++;osc1_type%=3; }
        else if(ch=='g') { osc2_type++;osc2_type%=3; }
        else if(ch=='b') { osc3_type++;osc3_type%=3; }
        else if(ch=='m') { third_mod++;third_mod%=3; }


        t += 1.0/S_RATE;
        float p1 = osc1*2*M_PI*t+osc1_t;
        float p2 = osc2*2*M_PI*t+osc2_t;
        float p3 = osc3*2*M_PI*t+osc3_t;

        I_t += 0.0001*sign;
        if(I_t>100||I_t<0) sign=-sign;

        if(third_mod==0) {
            float c1=cos(p1+osc1_t);
            if(osc1_type==1) c1 = c1<=0?-1:1;
            if(osc1_type==2) c1 = 1/osc1 * (osc1 - abs(index % (int) (osc1*2) - osc1) );
            float c2=cos(p2+I_t*c1+osc2_t);
            if(osc2_type==1) c2 = c2<=0?-1:1;
            if(osc2_type==2) c2 = 1/osc2 * (osc2 - abs(index % (int) (osc2*2) - osc2) );
            buffer[index]=A*c2;
        }
            ...
            if(osc2_type==2) c1 = 1/osc2 * (osc2 - abs(index % (int) (osc2*2) - osc2) );
            float c2=cos(p1+I_t*c1);
            if(osc1_type==1) c2 = c2<=0?-1:1;
            if(osc1_type==2) c2 = 1/osc1 * (osc1 - abs(index % (int) (osc1*2) - osc1) );
            float c3=cos(p3+I_t*c2);
            if(osc3_type==1) c3 = c3<=0?-1:1;
            if(osc3_type==2) c3 = 1/osc3 * (osc3 - abs(index % (int) (osc3*2) - osc3) );
            buffer[index]=A*c3;
        }
        else if(third_mod==2) {
            float c1=cos(p2);
        ...
        if(index%B_SIZE==0) {
            if(third_mod>0) {
                fprintf(stderr,"osc1=%02.2f/%d\tosc2=%02.2f/%d\tosc3=%02.2f/%d\tt_c=%02.2f\tt_m=%02.2f\tt_ck=%02.2f\tI_t=%02.2f\t%s\r\n",
                    ...
                    third_mod==0?"osc1->osc2":third_mod==1?"osc1->osc2->osc3":"osc1->osc2<-osc3");
            }
            else {
                fprintf(stderr,"osc1=%02.2f/%d\tosc2=%02.2f/%d\tt_c=%02.2f\tt_m=%02.2f\tI_t=%02.2f\t%s\r\n",
                    osc1,osc1_type,osc2,osc2_type,osc1_t,osc2_t,I_t,
                    "osc2->osc1");
            }
